<script lang="ts">
 ...
 let mapMode=$state<'tiles'|'wall'|'door'|'height'|'stairs'>('tiles');
 ...
 function paint(cells:Cell[],tile:MapBrush){
  try{const next=paintTiles(draft,cells,tile);parseAdventure({...bundle(),maps:bundle().maps.map(m=>m.id===draft.id?next:m)});if(JSON.stringify(next)===JSON.stringify(draft))return;snapshot();draft=next;notice=`Mapa actualizado en ${cells.length} casillas. Puedes deshacer el trazo.`;error='';}
  catch(e){error=(e as Error).message;}
 }
...
<div class="editor">
 ...
 <div class="editor-body">
  {#if toolPanel}<aside use:draggablePanel={{key:"tools",positions:panelPositions}} class="editor-objects" class:collapsed={toolCollapsed} aria-label="Herramienta activa"><div class="panel-heading"><h2>{toolPanel==='adventures'?'Aventuras':toolPanel==='maps'?'Mapas de la aventura':toolPanel==='objects'?'Objetos del mapa':toolPanel==='catalog'?'Añadir objeto':toolPanel==='paint'?'Editar mapa':'Conectar mapas'}</h2><div class="panel-actions"><button aria-label={toolCollapsed?"Expandir herramienta":"Minimizar herramienta"} aria-expanded={!toolCollapsed} onclick={()=>toolCollapsed=!toolCollapsed}>{#if toolCollapsed}<ChevronDown size={18}/>{:else}<ChevronUp size={18}/>{/if}</button><button aria-label="Cerrar herramienta" onclick={()=>showTool(null)}><X size={18}/></button></div></div><div hidden={toolCollapsed}>
   ...
   {:else if toolPanel==='paint'}   <section class="tile-palette" aria-label="Editar terreno"><div class="paint-actions">{#each [['tiles','Baldosas'],['wall','Paredes'],['door','Puertas'],['height','Altura'],['stairs','Escaleras']] as [id,label]}<button aria-pressed={mapMode===id} onclick={()=>{mapMode=id as 'tiles'|'wall'|'door'|'height'|'stairs';brush=null;wallTool=id==='wall'||id==='door'?id:null;selectedWall=null;}}>{label}</button>{/each}</div>{#if mapMode==='tiles'}<h3>Baldosas</h3><p>Elige una baldosa y pinta pulsando o arrastrando sobre el mapa.</p><div class="tile-options">{#each tileChoices as tile}<button aria-pressed={brush===tile.id} onclick={()=>{wallTool=null;brush=tile.id;testing=false;notice=`Pincel: ${tile.label}. Arrastra para pintar; deshacer revierte el trazo.`;}}>{#if tile.id==='void'}<span class="empty-tile" aria-hidden="true"></span>{:else}<img src={graphics.tiles[tile.id]} alt=""/>{/if}<span>{tile.label}</span></button>{/each}</div><div class="paint-actions"><button aria-pressed={brush==='erase'} onclick={()=>{wallTool=null;brush='erase';testing=false;}}>Restaurar suelo</button><button disabled={brush===null} onclick={()=>{brush=null;notice='Pincel desactivado.';}}>Dejar de pintar</button></div><p>{brush===null?'Pincel desactivado':brush==='erase'?'Restaurar: recupera el suelo original de cada casilla.':brush==='void'?'Vacío: elimina casillas libres. Mueve primero los objetos y accesos.':'Pincel activo. Puedes pintar también debajo de los objetos.'} Las casillas vacías no se pueden atravesar.</p>
 {:else if mapMode==='height'}<h3>Altura del terreno</h3><p>Elige un nivel y pinta una zona. Cada nivel sube 24 píxeles. Los objetos necesitan toda su base a la misma altura.</p><div class="paint-actions">{#each [-2,-1,0,1,2] as level}<button aria-pressed={brush===`height:${level}`} onclick={()=>{brush=`height:${level}`;wallTool=null;}}>Nivel {level}</button>{/each}</div><p>Para elevar una zona con objetivos, coloca antes su escalera de acceso. Puedes deshacer cada trazo.</p>
 {:else if mapMode==='stairs'}<h3>Escaleras</h3><p>Coloca la escalera en la baldosa inferior, apuntando hacia una baldosa un nivel más alta. Se entra por abajo y se sale por arriba; los lados no se atraviesan.</p><div class="paint-actions">{#each [['se','↘ Sureste'],['sw','↙ Suroeste'],['nw','↖ Noroeste'],['ne','↗ Noreste']] as [direction,label]}<button aria-pressed={brush===`stairs:${direction}`} onclick={()=>{brush=`stairs:${direction}` as MapBrush;wallTool=null;}}>{label}</button>{/each}<button aria-pressed={brush==='stairs:erase'} onclick={()=>{brush='stairs:erase';wallTool=null;}}>Quitar escalera</button></div><p>Una escalera conecta dos niveles consecutivos. No coloques objetos sobre los peldaños.</p>
