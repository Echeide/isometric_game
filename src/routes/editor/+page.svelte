<script lang="ts">
 ...
 async function playSaved(){
  try{const target=saveMap(localStorage,draft,slot);error='';await goto(`/?world=${target}`);}
 ...
 function changeWorld(value:string){
  try{saveMap(localStorage,draft,slot);const saved=readMaps(localStorage);snapshot();slot=value as WorldSlot;
